...
export default function PlagiarismReview() {
  ...
  const fetchReport = async () => {
    setLoading(true);
    try {
      const res = await API.get(`/plagiarism/contests/${contestId}/plagiarism-report`);
      setReport(res.data);
    } catch (err) {
        ...
        <div className="pr-inner">
          ...
          {loading ? (
            <div className="pr-stats">
              {[0,1,2].map(i => <div key={i} className="pr-shimmer" style={{height:80}}/>)}
            </div>
          ) : (
          ...
          {loading ? (
            [0,1].map(i => (
              <div key={i} style={{marginBottom:24}}>
                <div className="pr-shimmer" style={{height:60,marginBottom:4}}/>
                <div className="pr-shimmer" style={{height:240}}/>
              </div>
            ))
          ) : active.length === 0 ? (
