...
exports.getStats = catchAsync(async (req, res) => {
  ...
  // Try to get contest counts — graceful fallback if Contest model unavailable
  let totalContests  = 0;
  let activeContests = 0;
  try {
    const { Contest } = require("../models/Contest");
    const now = new Date();
    [totalContests, activeContests] = await Promise.all([
...
exports.getUsers = catchAsync(async (req, res) => {
  ...
  const result = users.map((u) => ({
    ...u,
    totalSolved: solvedMap[u._id.toString()] || 0,
    // Normalise ban field — model uses isActive but frontend expects status
...
exports.banUser = catchAsync(async (req, res) => {
  ...
  if (!user) throw new AppError("User not found", 404);
  res.json({ success: true, user: { ...user.toObject(), status: "banned" } });
});
...
exports.unbanUser = catchAsync(async (req, res) => {
  ...
  if (!user) throw new AppError("User not found", 404);
  res.json({ success: true, user: { ...user.toObject(), status: "active" } });
});
...
exports.approveProblem = catchAsync(async (req, res) => {
